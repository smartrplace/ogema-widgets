...
public interface GatewayTransferInfo extends PhysicalElement {
	...
	/**Data to be sent from client to master (some configuration and clean up actions can also be
	 * initiated by the master)
	 */
	ResourceList<ValueTransferInfo> valueData();
	
	/**Incident notification from client to master. It is usually up to the master to clean up events not
	 * needed anymore.*/
	ResourceList<IncidentProvider> incidentSpace();
	/**@deprecated use incidentSpace instead*/
	@Deprecated
