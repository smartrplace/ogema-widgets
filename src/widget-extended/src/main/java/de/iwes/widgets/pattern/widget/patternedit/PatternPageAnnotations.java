 ...
 * 
 * Inject the service into your application (must use a class which defines a {@see org.apache.felix.scr.annotations.Component} 
 * itself for this): <br>
 * 
 * <code>
 ...
 * </code>
 * 
 * or use {@see BundleContext#getService(org.osgi.framework.ServiceReference)}.
 * 
 * @author cnoelle
 * 
 */
...
public interface PatternPageAnnotations {
	 ...
	 * If this annotation is not present, it defaults to true for {@see SingleValueResource}s 
	...
	/**
	 * Additional annotation type for fields of a {@link ResourcePattern}. <br>
	 * If this annotation is not present, {@link #setAsReference()} defaults to false for {@see SingleValueResource}s 
	 ...
	 * 
	 * For {@see StringResource}s, the field {@link #enumType()} specifies a set of allowed values in terms of enum constants.
	 ...
	 * This also works for {@see ArrayResource}s. 
	 */
	@Target(ElementType.FIELD)
	@Retention(RetentionPolicy.RUNTIME)
	public @interface EntryType {
		...
		/**
		 * Mainly relevant to fields of type {@see TimeResource}. 
	 ...
	 * For a {@see ResourceList} field (which is not itself set as a reference) this annotation is compulsory,
