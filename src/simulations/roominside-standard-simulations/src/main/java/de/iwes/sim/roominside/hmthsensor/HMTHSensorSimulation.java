 ...
 */
public class HMTHSensorSimulation extends RoomInsideSimBase<HMTHSensorConfigPattern, HMTHSensorPattern> {
	...
	private final Map<String,SimulatedTHSens> simulatedObjects = new HashMap<>();
	private final RoomSimulationService roomSimService;
	
	public HMTHSensorSimulation(ApplicationManager am, RoomSimulationService roomSimService) {
		super(am, HMTHSensorPattern.class, false, HMTHSensorConfigPattern.class);
		this.roomSimService = roomSimService;
	}	

	@Override
	public String getProviderId() {
		return "HM TH-Sensor simulation _Room Simulation_";
	}
	...
	@Override
	public void buildConfigurations(HMTHSensorPattern pattern, List<SimulationConfiguration> cfgs, HMTHSensorConfigPattern simPattern) {
	...
	@Override
	public void buildQuantities(HMTHSensorPattern pattern, List<SimulatedQuantity> quantities, HMTHSensorConfigPattern simPattern) {
		// Add here configuration values that can be edited by the user, see example below
	}
	
	@Override
	public String getDescription() {
		return "Simulated temperature/humidity sensor connected into room via Room Simulation Service."
				+ " Note that after creation of a simulated object the room connector has to be set before"
				+ "the simulation can be activated.";
	}
	
	@Override
	public void simTimerElapsed(HMTHSensorPattern targetPattern, HMTHSensorConfigPattern configPattern, Timer t, long timeStep) {
	...
	@Override
	protected void initSimulation(HMTHSensorPattern targetPattern, HMTHSensorConfigPattern configPattern) {
		// initialize and activate also the optional fields in targetPattern
		boolean finalActivate = false;
		if(setIfNewOrNaN(targetPattern.humidity, 0.49f) |
				ValueResourceHelper.setIfNewCelsius(targetPattern.temperature, 20.5f) |
		...
		SimulatedTHSens logic = new SimulatedTHSens(appManager, targetPattern, configPattern,
				roomSimService, this);
		simulatedObjects.put(targetPattern.model.getLocation(), logic);
	}
	
	public static boolean setIfNewOrNaN(FloatResource fres, float value) {
		if((!fres.exists()) || Float.isNaN(fres.getValue())) {
			fres.create();
			fres.setValue(value);
			return true;
		}
	...
	@Override
	protected void removeSimulation(HMTHSensorPattern targetPattern, HMTHSensorConfigPattern configPattern) {
		SimulatedTHSens logic = simulatedObjects.remove(targetPattern.model.getLocation());
		logic.close();
	}
