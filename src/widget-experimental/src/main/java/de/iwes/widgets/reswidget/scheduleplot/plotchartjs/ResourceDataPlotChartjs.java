...
public class ResourceDataPlotChartjs extends ResourceData<ChartjsDataSet> {

	@Override
	protected ChartjsDataSet getData(String id,Map<Long,Float> valueMap) {
	...
	private static JSONArray getJSONData(Map<Long,Float> valueMap) {
		...
		for (Map.Entry<Long, Float> entry : valueMap.entrySet()) {
			...
			json.put("x", entry.getKey());
			json.put("y", entry.getValue());
			array.put(json);
		}
		return array;
	}
